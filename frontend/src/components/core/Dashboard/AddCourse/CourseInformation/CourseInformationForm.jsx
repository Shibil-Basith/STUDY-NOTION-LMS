...
export default function CourseInformationForm() {
  ...
  useEffect(() => {
    ...
    // if form is in edit mode, pre-fill values
    if (editCourse && course) {
      ...
      // 🔴 IMPORTANT: store only the category _id, not the whole object
  ...
  const isFormUpdated = () => {
    ...
    const originalCategoryId = course?.category?._id || ""
    ...
    if (
      ...
      // ✅ Compare string id with string id
  ...
  const onSubmit = async (data) => {
    ...
    if (editCourse) {
      if (isFormUpdated()) {
        ...
        const originalCategoryId = course?.category?._id || ""
        if (currentValues.courseCategory !== originalCategoryId) {
    ...
    formData.append("tag", JSON.stringify(data.courseTags))
    formData.append("whatYouWillLearn", data.courseBenefits)
    // ✅ category is the selected category _id
    formData.append("category", data.courseCategory)
